...
| QUIC stream handling | The two-byte DoQ length prefix parser reads from a bounded Low* buffer, the first complete length-prefix transition is implemented, and `find_stream` performs a conservative first-slot lookup; stream accumulation/body copying plus stream allocation/close are still incomplete or admitted. | Resource exhaustion and stream confusion defenses are not yet proven. | Implement stream accumulation, allocation, close, and resource-bound proofs. |
