...
| LowParse/EverParse generated boundary | The local LowParse shim has been removed. The parser boundary now has a production-target subset entry point, a checked-in 3D grammar seed for bounded uncompressed-QNAME question validation, opt-in generation and verification targets, containerized EverParse/3D tooling, CI generation of the subset artifact, an EverParse-verified adapter that imports the generated validator symbols, and an active Low* buffer gate through the generated C wrapper for question-only packets with bounded uncompressed QNAMEs. Full packet construction still uses the handwritten reference parser. | Parser proofs still rely on a hand-authored generated-boundary shim outside the generated question subset, and the F* interface for the generated C wrapper is trusted. | Expand the grammar into RR-section validation, replace reference construction or prove equivalence, and keep the generated C-wrapper interface aligned with generated artifacts. |
