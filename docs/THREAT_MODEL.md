...
| QUIC/TLS shell stack | TLS 1.3, QUIC packet protection, key updates, connection lifecycle, recovery, congestion control, stream delivery, flow control, socket I/O, and event-loop behavior are delegated to MsQuic as the preferred maintained implementation in the unverified shell. `DNS.QUIC.MsQuicIngress.handle_authenticated_stream_fragment` is the first verified ingress boundary for authenticated stream bytes, and `DNS.QUIC.MsQuicEgress.prepare_response_send` defines the Low* egress handoff for response bytes copied into a caller-provided worker response buffer. | TLS authenticity, AEAD integrity, certificate/authentication policy, QUIC transport correctness, response-buffer lifetime/aliasing, send completion, and close sequencing are trusted properties of MsQuic and the shell adapter, not verified in this repository. The F* tokens only record the boundary; they do not prove MsQuic behavior. | Pin, audit, and fuzz the MsQuic integration; keep the shell/core contract limited to authenticated stream bytes and response bytes. |
