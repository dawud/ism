...
| DNS parser proof debt | Some parser proofs still use `admit()` or `assume`. | Parser-rejecting claims may be stronger than the checked proof. | Remove parser admits/assumes and add named safety lemmas for header, question, and buffer parsing. |
