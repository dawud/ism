...
| QUIC stream handling | The two-byte DoQ length prefix parser reads from a bounded Low* buffer, but stream accumulation and multiplexer operations are still admitted. | Resource exhaustion and stream confusion defenses are not yet proven. | Implement stream accumulation, allocation, close, and resource-bound proofs. |
