...
| LowParse/EverParse bootstrap adapter | `spec/LowParse.Low.Base.fsti` documents a trusted adapter for `uint8_ptr` and the unused placeholder `parser` type. The parser boundary now has a production-target subset entry point, a checked-in 3D grammar seed for the DNS header/root-question subset, an opt-in `make everparse-generate` target, and containerized EverParse/3D tooling, but generated EverParse/LowParse output is not wired into the active parser yet. | Parser proofs rely on a local LowParse surface and hand-authored generated-boundary shim until the real EverParse/LowParse dependency is wired in. | Generate the F*/C validator from `everparse/DNSProtocol.3d`, replace the shim with the generated module, then expand the grammar beyond the header/root-question subset. |
