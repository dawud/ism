...
| QUIC stream handling | The two-byte DoQ length prefix parser reads from a bounded Low* buffer, and the first complete length-prefix transition is implemented; stream accumulation/body copying and multiplexer operations are still incomplete or admitted. | Resource exhaustion and stream confusion defenses are not yet proven. | Implement stream accumulation, allocation, close, and resource-bound proofs. |
