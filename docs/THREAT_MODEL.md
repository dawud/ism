...
| QUIC/TLS shell stack | TLS 1.3, QUIC packet protection, key updates, connection lifecycle, recovery, congestion control, stream delivery, flow control, socket I/O, and event-loop behavior are delegated to MsQuic as the preferred maintained implementation in the unverified shell. | TLS authenticity, AEAD integrity, certificate/authentication policy, and QUIC transport correctness are trusted properties of MsQuic and the shell adapter, not verified in this repository. | Pin, audit, and fuzz the MsQuic integration; keep the shell/core contract limited to authenticated stream bytes and response bytes. |
