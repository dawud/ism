...
| LowParse/EverParse generated boundary | The local LowParse shim has been removed. The parser boundary now has a production-target subset entry point, a checked-in 3D grammar seed for bounded uncompressed-QNAME question validation, a single-answer RR packet subset, A/AAAA fixed-RDLENGTH answer subsets, NS/CNAME/PTR name-RDATA answer subsets, an MX preference/exchange-name answer subset, an SOA two-name/timer answer subset, and an SRV priority/weight/port/target-name answer subset, opt-in generation and verification targets, containerized EverParse/3D tooling, CI generation of the subset artifact, an EverParse-verified adapter that imports the generated validator symbols, and an active Low* buffer gate through the generated C wrapper for question-only packets with bounded uncompressed QNAMEs plus one-question/one-answer packets with bounded uncompressed names, raw bounded RDATA, generated A/AAAA length checks, generated name-RDATA shape checks, generated MX shape checks, generated SOA shape checks, and generated SRV shape checks. Full packet construction still uses the handwritten reference parser. | Parser proofs still rely on a hand-authored generated-boundary shim outside the generated subset, and the F* interface for the generated C wrapper is trusted. | Expand the grammar across the remaining RR-section forms and type-specific RDATA shapes, replace reference construction or prove equivalence, and keep the generated C-wrapper interface aligned with generated artifacts. |
...
| DNS parser completeness | Header, question, buffer, and flag round-trip parser obligations are admitted-free; RR sections are parsed structurally with bounded RDATA preservation, the generated boundary gates question-only packets, the first single-answer RR subset, A/AAAA fixed-RDLENGTH answer subsets, NS/CNAME/PTR name-RDATA answer subsets, MX preference/exchange-name answer subsets, SOA two-name/timer answer subsets, and SRV priority/weight/port/target-name answer subsets, unknown TYPE values are preserved as `UNKNOWN`, truncated RR headers/RDATA are rejected, invalid A/AAAA RDATA lengths are rejected, NS/CNAME/PTR RDATA must be a single uncompressed name, MX RDATA must be a preference plus one uncompressed exchange name, SOA RDATA must be two uncompressed names plus five 32-bit timer fields, TXT RDATA must contain one or more fully-consumed character strings, SRV RDATA must contain priority, weight, port, and one uncompressed target name, EDNS0 OPT pseudo-RRs/options are accepted only in version-0 additional-section form with bounded option data, and basic response/EDNS0 option/padding packet serialization validates section counts and round-trips question-only packets through the parser. | Parser-rejecting claims currently cover structural RR parsing, generated A/AAAA length checks, generated NS/CNAME/PTR name-RDATA shape checks, generated MX shape checks, generated SOA shape checks, generated SRV shape checks, TXT shape checks, structural OPT/EDNS option checks, and basic response/OPT/Padding packet serialization, not broader type-specific RDATA semantics, full response-side EDNS policy integration, or compression-pointer support. | Implement remaining type-specific RDATA validation, response-side EDNS policy integration, and compression support or keep each unsupported behavior explicitly rejected, then extend parser-rejecting proofs and shared parser tests. |
