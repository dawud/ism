...
| LowParse/EverParse generated boundary | The local LowParse shim has been removed. The parser boundary now has a production-target subset entry point, a checked-in 3D grammar seed for the DNS header/root-question subset, opt-in generation and verification targets, containerized EverParse/3D tooling, CI generation of the subset artifact, and an EverParse-verified adapter that imports the generated validator symbols, but generated EverParse/LowParse output is not used for active packet construction yet. | Parser proofs still rely on a hand-authored generated-boundary shim until the generated LowParse validator is connected to the active Low* buffer path. | Replace the shim with a generated-validator Low* adapter, then expand the grammar beyond the header/root-question subset. |
