...
| QUIC stream handling | The two-byte DoQ length prefix parser reads from a bounded Low* buffer, the first complete length-prefix transition is implemented, `find_stream` performs a conservative first-slot lookup, `allocate_stream` is an explicit no-allocation placeholder, and `close_stream` is an explicit no-op placeholder; stream accumulation/body copying plus real allocation and close/removal semantics are still incomplete. | Resource exhaustion and stream confusion defenses are not yet proven. | Implement stream accumulation, allocation, close/removal, and resource-bound proofs. |
