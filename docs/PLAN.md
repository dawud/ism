...
- **Sub-phases:**
  ...
  - **2F: MsQuic shell integration:** Wire MsQuic as the preferred maintained QUIC/TLS implementation in the unverified shell, keeping its trust assumptions visible. Current work defines ingress and egress F* handoff boundaries, routes worker response bytes into a caller-provided Low* response buffer, wraps response bytes with the RFC 9250 two-octet DoQ length prefix before preparing a MsQuic send descriptor, exposes a send-completion/drop cleanup boundary that closes the stream, adds generated C ingress, minimal worker response, scheduler-helper, response handoff/completion, and stream-reset/drop ABIs, adds a verified shell-event dispatcher over those boundaries, adds a fixed-capacity C shell scaffold that owns connection/stream buffers over the generated ABIs, adds a MsQuic-shaped C adapter scaffold whose fake callback smoke coverage exercises validated question-echo and FORMERR responses plus single-send in-flight tracking and reset/drop cleanup, adds a fixed-capacity C shell event queue that services derived ready responses synchronously when ingress reaches `Processing`, adds a real-MsQuic callback seam that copies `QUIC_STREAM_EVENT` receive bytes into shell-owned ingress storage before queueing and maps send-completion/reset events into that queue/adapter path, and checks that callback seam against a pinned upstream `msquic.h` header in CI; real MsQuic library linkage, polling, rich-dispatcher C ABI coverage, and production event-loop integration remain.
- **Tasks:**
...
The extraction gate verifies all scaffold modules and sends the current protocol/security/transport boundary plus `DNS.Worker.Minimal` and the shell-event dispatcher path to KaRaMeL. The clean emitted C/link surface covers the protocol/EverParse parser boundary, `DNS.ShellBoundary.dispatch_authenticated_stream_data`, `DNS.ShellBoundary.process_ready_stream_for_response`, `DNS.ShellBoundary.process_ready_stream_for_empty_response`, `DNS.ShellBoundary.process_ready_stream_for_validated_minimal_response`, `DNS.ShellBoundary.*_via_scheduler` C-shaped scheduler helpers, and `DNS.ShellResponseBoundary` response send handoff/completion plus DoQ egress framing. The full authoritative `DNS.Worker` parser/serializer/zone path, verification-only parser tests, and the broader Phase 3/4 cache/concurrency scaffolds stay out of extraction until they are rewritten into Low* or explicitly marked as trusted/specification-only boundaries.
...
The C link smoke gate links and runs a tiny harness against the current extracted protocol bundle, generated EverParse wrapper, shell ingress boundary, minimal worker FORMERR, header-only empty NOERROR, validated question-echo response boundaries, DoQ egress response framing, C-shaped scheduler helpers, response send handoff/completion boundary, stream reset/drop boundary, fixed-capacity C shell scaffold, MsQuic-shaped adapter scaffold over fake callback data for validated question-echo, invalid FORMERR, busy-send rejection, mismatched-completion rejection, reset-before-response, reset-during-send, and repeated-reset cases, the fixed-capacity C shell event queue with synchronous ready-response service for completed ingress, and the dependency-free MsQuic runtime receive-copy/send-completion/reset seam with overflow rejection. It does not link the rich `DNS.ShellScheduler.dispatch_shell_event` union or a real MsQuic library path yet; `make msquic-runtime-compile-smoke` syntax-checks the `QUIC_STREAM_EVENT` wrapper against the pinned upstream `msquic.h` header installed in the stable container.

The remaining warning-15 debt is concentrated in the protocol model:
- DoQ stream accumulation, including the two-octet length-prefix byte widening,
  state transitions, body-length accounting, bounded copy lengths,
  stored-prefix state, and worker processing-length handoff, no longer appears
  as separate warning-15 entries;
...
Response-handoff, scheduler send-completion, minimal worker error-response
construction, shell/scheduler minimal-worker dispatch, stream lookup/close,
stream accumulation state/copy helpers, and DoQ length-prefix byte widening now
use machine-integer-friendly code and no longer appear as separate warning-15
entries.
...
| [RFC 9250](https://datatracker.ietf.org/doc/html/rfc9250) | DoQ framing | Two-octet length prefix | Partial | Low* stream state verifies complete and split length-prefix parsing, bounded body copying, `ReadingMessage` progress to `Processing` with the completed DNS message length, conservative overlong-fragment rejection, bounded active stream lookup, capacity-bounded stream allocation, compacting active-stream close, worker dispatch through verified stream lookup, worker-side completed-buffer parsing into response bytes, capacity-checked response copy into a caller-provided Low* buffer, verified DoQ egress framing that writes the two-octet response length prefix into a caller-owned stream buffer, worker preparation of a MsQuic send descriptor for that buffer, send-completion/drop cleanup that closes the stream, a verified shell-event dispatcher over authenticated ingress, processing-ready, and send-completion/drop events, emitted `DNS.ShellBoundary.dispatch_authenticated_stream_data`, `DNS.ShellBoundary.process_ready_stream_for_response`, `DNS.ShellBoundary.process_ready_stream_for_empty_response`, `DNS.ShellBoundary.process_ready_stream_for_validated_minimal_response`, `DNS.ShellBoundary.dispatch_stream_reset_via_scheduler`, and `DNS.ShellBoundary.*_via_scheduler` C ABIs for authenticated ingress, minimal 12-byte FORMERR worker error-response, header-only empty NOERROR response construction, generated-validator-backed question-echo response selection, stream reset/drop cleanup, and scheduler-helper coverage, emitted `DNS.ShellResponseBoundary` C ABIs for response send handoff/completion and DoQ egress framing, a fixed-capacity C shell scaffold over those generated ABIs, a MsQuic-shaped adapter scaffold with fake callback smoke coverage for validated question-echo, invalid FORMERR, busy-send rejection, mismatched-completion rejection, reset-before-response, reset-during-send, and repeated-reset cases, a fixed-capacity C shell event queue with FIFO/overflow and synchronous ready-response/reset smoke coverage, and a CI syntax gate that checks the real `QUIC_STREAM_EVENT` callback seam against pinned upstream `msquic.h`. | Actual MsQuic send-path wiring, real MsQuic library linkage, rich-dispatcher C symbols, response-buffer lifetime/aliasing proofs beyond the single tracked shell send slot, real polling, C scheduler integration, production event-loop integration, and resource-bound proofs remain incomplete. |
