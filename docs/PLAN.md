...
| [RFC 9250](https://datatracker.ietf.org/doc/html/rfc9250) | DoQ framing | Two-octet length prefix | Partial | Low* stream state verifies complete and split length-prefix parsing, bounded body copying, and `ReadingMessage` progress to `Processing`. | Real overlong-body/resource policy is still incomplete. |
