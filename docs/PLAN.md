...
The C link smoke gate links and runs a tiny harness against the current extracted protocol bundle, generated EverParse wrapper, shell ingress boundary, minimal worker FORMERR, header-only empty NOERROR, validated question-echo response boundaries, DoQ egress response framing, C-shaped scheduler helpers, response send handoff/completion boundary, fixed-capacity C shell scaffold, MsQuic-shaped adapter scaffold over fake callback data for both validated question-echo and invalid FORMERR response cases, the fixed-capacity C shell event queue with synchronous ready-response service for completed ingress, and the dependency-free MsQuic runtime receive-copy/send-completion seam with overflow rejection. It does not link the rich `DNS.ShellScheduler.dispatch_shell_event` union or a real MsQuic library path yet; `make msquic-runtime-compile-smoke` syntax-checks the optional `QUIC_STREAM_EVENT` wrapper when MsQuic headers are provided.
