...
| [RFC 1035](https://datatracker.ietf.org/doc/html/rfc1035) | Header format | 12-byte DNS header | Partial | Pure parser tests cover valid and truncated headers; serializer tests cover response header bytes; Low* buffer boundary reads the checked length; shared fixtures now exercise the EverParse production-target subset boundary. | External generated parser not integrated yet. |
