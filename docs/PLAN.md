...
| :--- | :--- | :--- | :--- | :--- | :--- |
| [RFC 1034](https://datatracker.ietf.org/doc/html/rfc1034) | DNS concepts | Authoritative data, recursion, CNAME, and wildcard model | Partial | Exact radix lookup and CNAME hop-count scaffolds exist; the zone parser accepts one bootstrap binary zone-entry shape and rejects truncated, trailing, and invalid A/AAAA RDATA-length entries. | Full master-file text parsing, multi-entry zone loading, wildcard semantics, full CNAME chasing, and recursive behavior are incomplete. |
