...
- **Sub-phases:**
  ...
  - **2F: MsQuic shell integration:** Wire MsQuic as the preferred maintained QUIC/TLS implementation in the unverified shell, keeping its trust assumptions visible. Current work defines ingress and egress F* handoff boundaries and routes worker response bytes into a caller-provided Low* response buffer before preparing a MsQuic send descriptor; C shell wiring, send-completion cleanup, and close sequencing remain.
- **Tasks:**
