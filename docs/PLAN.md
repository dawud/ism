...
| [RFC 8446](https://datatracker.ietf.org/doc/html/rfc8446) | TLS 1.3 | Authenticated transport | Trusted | AEAD decrypt now branches on a trusted authenticated-decrypt adapter instead of unconditional success. | Client hello validation is still mocked, and AEAD authenticity still depends on a trusted adapter rather than real EverCrypt integration. |
