...
The remaining warning-15 debt is concentrated in the stream length-prefix
decoder and the protocol model:
- DoQ stream accumulation still uses one byte-to-`UInt32` helper for the
  two-octet length prefix, but state transitions, body-length accounting,
  bounded copy lengths, stored-prefix state, and worker processing-length
  handoff no longer appear as separate warning-15 entries;
...
construction, shell/scheduler minimal-worker dispatch, stream lookup/close, and
stream accumulation state/copy helpers now use machine-integer-friendly code
and no longer appear as separate warning-15 entries.
