...
The C link smoke gate links and runs a tiny harness against the current extracted protocol bundle and generated EverParse wrapper. It does not link the worker/dispatcher path or any MsQuic shell code yet.
...
Extraction status: containerized `make extract` is now a CI smoke gate. It verifies all F*/spec modules first, then sends the current protocol/security/transport boundary plus `DNS.Zone.RadixTree`, `DNS.Worker`, and `DNS.ShellScheduler` to KaRaMeL; broader Phase 3/4 cache/concurrency scaffolds remain verification-only. `make c-compile-smoke` syntax-checks the extracted C bundle and EverParse wrapper, and `make c-link-smoke` links and runs the current emitted protocol/EverParse boundary without linking a final shell binary.
