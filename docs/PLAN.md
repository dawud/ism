...
The C link smoke gate links and runs a tiny harness against the current extracted protocol bundle, generated EverParse wrapper, shell ingress boundary, minimal worker FORMERR, header-only empty NOERROR, and validated minimal response boundaries, C-shaped scheduler helpers, response send handoff/completion boundary, fixed-capacity C shell scaffold, and MsQuic-shaped adapter scaffold over fake callback data. It does not link the rich `DNS.ShellScheduler.dispatch_shell_event` union or real MsQuic shell code yet.
