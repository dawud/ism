...
| [RFC 9250](https://datatracker.ietf.org/doc/html/rfc9250) | DoQ framing | Two-octet length prefix | Partial | Low* stream state verifies complete and split length-prefix parsing, bounded body copying, `ReadingMessage` progress to `Processing`, conservative overlong-fragment rejection, and conservative first-slot stream close/removal. | Real stream allocation and multi-slot close/removal semantics are still incomplete. |
