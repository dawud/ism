...
- [ ] **Validation:** Prove that response generation never leaks cross-thread memory. A minimal pure response packet builder now echoes request questions, sets response flags/RCODEs, and serializes parser-round-trippable empty-answer responses; resolver-backed response construction, worker integration, and cross-thread memory proofs are not present.
