...
- [/] Implement `DNS.QUIC.StreamMapping` state machine (ReadingLength, ReadingMessage). State types exist, the two-byte DoQ length prefix parser reads a bounded Low* buffer, `handle_stream_data` copies bounded body bytes into the stream buffer, persists stream phase updates, stores a one-byte partial length prefix, accounts for body bytes after a completed length prefix, advances `ReadingMessage` to `Processing` once enough bytes arrive, and transitions to `Done` for overlong body fragments. Full multi-slot allocation and close/removal semantics are still incomplete.
- [/] Implement Stream ID Multiplexer to handle concurrent streams. `find_stream` performs a verified conservative first-slot lookup with explicit ownership preconditions; `allocate_stream` conservatively reuses the first slot when the connection is empty and resets it to `ReadingLength 0ul`; and `close_stream` conservatively removes a matching first active stream by setting the active count to zero. Multi-slot allocation and close/removal semantics are still incomplete.
...
- [/] Implement the Worker Thread harness (`worker_loop`). Current harness performs a conservative first-slot stream lookup, reads the stream context directly, moves matching `Processing` streams to `Done` without admits, and invokes conservative first-slot `close_stream` cleanup. Real response generation, polling, scheduler integration, and multi-slot cleanup remain incomplete.
