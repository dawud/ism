...
- [ ] **Validation:** Prove that response generation never leaks cross-thread memory. A pure authoritative response adapter now maps parsed requests through lookup/CNAME resolution into response packets, the response packet builder echoes request questions, maps success records to the answer section, maps error results to empty-answer responses with the selected RCODE, and worker helpers parse completed stream buffers and serialize response bytes; worker-loop integration, egress handoff, and cross-thread memory proofs are not present.
...
- [/] Implement the Worker Thread harness (`worker_loop`). Current harness uses verified stream lookup, reads a matching active stream context, and invokes verified `close_stream` cleanup for `Processing` streams without admits. A read-only helper can parse a completed stream buffer and build response bytes, but `worker_loop` response byte dispatch, polling, and scheduler integration remain incomplete.
