...
- [ ] Remove all remaining `admit()` calls from multiplexer allocation/close operations, recursive cache operations, sharded cache operations, and worker processing.
...
- [/] Implement Stream ID Multiplexer to handle concurrent streams. `find_stream` performs a verified conservative first-slot lookup with explicit ownership preconditions; `allocate_stream` and `close_stream` are still admitted.
