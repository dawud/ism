...
- [ ] Execute parser strategy decision:
  ...
  - [/] Integrate an EverParse-generated parser/serializer as the production target. Current work adds a production-target boundary candidate for the implemented header/question subset, runs the shared parser fixtures through that boundary, checks in a 3D grammar seed for the DNS header/root-question subset, adds an explicit `make everparse-generate` target, and installs EverParse/3D tooling in the pinned container; generated parser output is not wired into the active F* parser yet.
...
- [/] Implement `DNS.Protocol.header_validator` and `header_reader` using EverParse. Current code has pure byte-list header/RR parsing, minimal full-packet byte serialization for uncompressed names/raw RDATA, a verified Low* buffer reader, an `EverParseBoundary` module that routes through a production-target subset boundary matching the reference parser, a checked-in 3D grammar seed plus generator target for the DNS header/root-question subset, and containerized EverParse/3D tooling; no external generated EverParse header reader is wired into the active parser yet.
