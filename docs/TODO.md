...
- [/] Implement `DNS.QUIC.StreamMapping` state machine (ReadingLength, ReadingMessage). State types exist, the two-byte DoQ length prefix parser reads a bounded Low* buffer, and `handle_stream_data` accounts for body bytes after a complete length prefix and advances `ReadingMessage` to `Processing` once enough bytes arrive. Split length-prefix accumulation and message-body copying are not implemented yet.
