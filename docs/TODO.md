...
- [ ] Execute parser strategy decision:
  ...
  - [/] Integrate an EverParse-generated parser/serializer as the production target. Current work adds a production-target boundary candidate for the implemented header/question subset, runs the shared parser fixtures through that boundary, checks in a 3D grammar seed for the DNS header/root-question subset, and adds an explicit `make everparse-generate` target; external EverParse generation is not available in the pinned container or wired into the active F* parser yet.
...
- [/] Implement `DNS.Protocol.header_validator` and `header_reader` using EverParse. Current code has pure byte-list header/RR parsing, minimal full-packet byte serialization for uncompressed names/raw RDATA, a verified Low* buffer reader, an `EverParseBoundary` module that routes through a production-target subset boundary matching the reference parser, and a checked-in 3D grammar seed plus generator target for the DNS header/root-question subset; no external generated EverParse header reader is present yet.
