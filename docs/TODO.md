...
- [/] Integrate EverCrypt for TLS 1.3 Handshake. Current `DNS.Security.Handshake` defines the state type and transitions, but `verify_client_hello` delegates success/failure to the trusted `EverCrypt.Cipher.validate_client_hello` bootstrap adapter; real miTLS/EverQuic integration remains incomplete.
...
- [/] Implement `DNS.QUIC.StreamMapping` state machine (ReadingLength, ReadingMessage). State types exist, the two-byte DoQ length prefix parser reads a bounded Low* buffer, `handle_stream_data` copies bounded body bytes into the stream buffer, persists stream phase updates, stores a one-byte partial length prefix, accounts for body bytes after a completed length prefix, advances `ReadingMessage` to `Processing` once enough bytes arrive, and transitions to `Done` for overlong body fragments. Response-generation integration and resource-bound proofs remain incomplete.
...
- [ ] **Validation:** Prove that response generation never leaks cross-thread memory. A pure authoritative response adapter now maps parsed requests through lookup/CNAME resolution into response packets, the response packet builder echoes request questions, maps success records to the answer section, maps error results to empty-answer responses with the selected RCODE, and a pure worker helper serializes those packets into response bytes; stream-buffer parsing, egress handoff, and cross-thread memory proofs are not present.
...
- [/] Implement the Worker Thread harness (`worker_loop`). Current harness uses verified stream lookup, reads a matching active stream context, and invokes verified `close_stream` cleanup for `Processing` streams without admits. Pure request-to-response byte construction exists for already-parsed requests, but stream-buffer parsing, response byte dispatch, polling, and scheduler integration remain incomplete.
