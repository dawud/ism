...
val find_stream_from :
  ...
  Stack (option (buffer stream_context))
    (requires (fun h0 -> active_streams_live h0 active count))
    (ensures (fun h0 _ h1 -> modifies_none h0 h1))
    (decreases (FStar.UInt32.v count - FStar.UInt32.v idx))

let rec find_stream_from active count id idx =
    ...
    begin
      ...
      if stream.sc_id = id then
        ...
        begin
          ...
          find_stream_from active count id next_idx
        end
    end
    ...
    ST (option (buffer stream_context))
      (requires (fun h0 ->
        ...
        (let c = FStar.Seq.index (LowStar.Buffer.as_seq h0 conn) 0 in
         ...
         active_streams_live h0 c.cc_active c.cc_num)))
      (ensures (fun h0 _ h1 -> modifies_none h0 h1))
...
let find_stream conn_ptr id =
  ...
  if FStar.UInt32.v conn.cc_num = 0 then
    ...
    find_stream_from conn.cc_active conn.cc_num id 0ul
...
let allocate_stream conn_ptr id =
  ...
  if FStar.UInt32.v conn.cc_num = 0 then
    ...
    LowStar.Buffer.upd conn_ptr 0ul { conn with cc_num = 1ul };
    Some stream_ptr
  else
    None
