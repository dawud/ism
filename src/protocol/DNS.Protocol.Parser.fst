...
val generated_uncompressed_question_two_a_answer_packet_fields :
  input:list FStar.UInt8.t ->
  Tot (option (generated_dns_name_length * generated_dns_name_length * generated_dns_name_length))

let generated_uncompressed_question_two_a_answer_packet_fields input =
  match parse_header_bytes input with
  | Some (h, after_header) ->
      if FStar.UInt16.v h.qdcount = 1 &&
         FStar.UInt16.v h.ancount = 2 &&
         FStar.UInt16.v h.nscount = 0 &&
         FStar.UInt16.v h.arcount = 0 then
        match DNS.Name.parse_qname 128 after_header with
        | Some (question_name, question_tail) ->
            ...
            if L.length question_tail < 4 then
              ...
              let (_, after_question) = L.splitAt 4 question_tail in
              begin match DNS.Name.parse_qname 128 after_question with
              | Some (first_rr_name, first_rr_tail) ->
                  DNS.Name.lemma_parser_rejecting 128 after_question;
                  let first_rr_name_length_nat =
                    DNS.Name.dns_name_length first_rr_name in
                  assert (first_rr_name_length_nat > 0);
                  assert (first_rr_name_length_nat <= 255);
                  begin match first_rr_tail with
                  | rt_hi :: rt_lo ::
                    _rc_hi :: _rc_lo ::
                    _ttl_0 :: _ttl_1 :: _ttl_2 :: _ttl_3 ::
                    rdlen_hi :: rdlen_lo ::
                    _rdata_0 :: _rdata_1 :: _rdata_2 :: _rdata_3 ::
                    after_first_answer ->
                      let rr_type = FStar.UInt16.v (u16_from_be rt_hi rt_lo) in
                      let rdata_length =
                        FStar.UInt16.v (u16_from_be rdlen_hi rdlen_lo) in
                      if rr_type = 1 && rdata_length = 4 then
                        begin match DNS.Name.parse_qname 128 after_first_answer with
                        | Some (second_rr_name, second_rr_tail) ->
                            DNS.Name.lemma_parser_rejecting 128 after_first_answer;
                            let second_rr_name_length_nat =
                              DNS.Name.dns_name_length second_rr_name in
                            assert (second_rr_name_length_nat > 0);
                            assert (second_rr_name_length_nat <= 255);
                            begin match second_rr_tail with
                            | rt2_hi :: rt2_lo ::
                              _rc2_hi :: _rc2_lo ::
                              _ttl2_0 :: _ttl2_1 :: _ttl2_2 :: _ttl2_3 ::
                              rdlen2_hi :: rdlen2_lo ::
                              _rdata2_0 :: _rdata2_1 :: _rdata2_2 :: _rdata2_3 :: [] ->
                                let rr2_type =
                                  FStar.UInt16.v (u16_from_be rt2_hi rt2_lo) in
                                let rdata2_length =
                                  FStar.UInt16.v (u16_from_be rdlen2_hi rdlen2_lo) in
                                if rr2_type = 1 && rdata2_length = 4 then
                                  let qn:generated_dns_name_length = qname_length_nat in
                                  let first:generated_dns_name_length =
                                    first_rr_name_length_nat in
                                  let second:generated_dns_name_length =
                                    second_rr_name_length_nat in
                                  Some (qn, first, second)
                                else
                                  None
                            | _ -> None
                            end
...
val generated_uncompressed_question_two_a_answer_packet_subset_applicable :
  input:list FStar.UInt8.t ->
  Tot bool

let generated_uncompressed_question_two_a_answer_packet_subset_applicable input =
  match generated_uncompressed_question_two_a_answer_packet_fields input with
  | Some (qname_length, first_rr_name_length, second_rr_name_length) ->
      qname_length > 0 &&
      qname_length <= 255 &&
      first_rr_name_length > 0 &&
      first_rr_name_length <= 255 &&
      second_rr_name_length > 0 &&
      second_rr_name_length <= 255
  | None -> false
...
val generated_compressed_name_rdata_packet_fields :
  input:list FStar.UInt8.t ->
  Tot (option (generated_dns_name_length * generated_dns_name_length * nat * nat * nat))
                ...
                if L.length question_tail < 4 then
                  ...
                  | Some (_, rr_tail) ->
                      ...
                      | _rt_hi :: _rt_lo ::
                        ...
                        _rdlen_hi :: _rdlen_lo ::
                        ptr_hi :: ptr_lo :: [] ->
                          let rdata_offset = 26 + qname_length + rr_name_length in
                          let rdata_name_pointer = [ptr_hi; ptr_lo] in
                          begin match DNS.Name.parse_qname_compressed
                                  128
                                  input
                                  rdata_offset
                                  rdata_name_pointer with
                          | Some (_, []) ->
                              let ptr_hi_nat = FStar.UInt8.v ptr_hi in
                              let ptr_lo_nat = FStar.UInt8.v ptr_lo in
                              if ptr_hi_nat >= 192 then begin
                                ...
                                Some (qn, rn, rr_type, ptr_hi_nat, ptr_lo_nat)
                              end
                              else
                                None
                          | _ -> None
...
let generated_compressed_name_rdata_packet_subset_applicable input =
  ...
  | Some (qname_length, rr_name_length, rr_type, ptr_hi, ptr_lo) ->
      qname_length > 0 &&
      qname_length <= 255 &&
      rr_name_length > 0 &&
      rr_name_length <= 255 &&
...
let generated_compressed_soa_packet_fields input =
  ...
  | Some (qname_length, rr_name_length, rdata_length, rr_type, _, _, _) ->
      if rr_type = 6 &&
         rdata_length >= 23 &&
         L.length input >= 49 + qname_length + rr_name_length then
        match parse_header_bytes input with
        | Some (_, after_header) ->
            begin match DNS.Name.parse_qname 128 after_header with
            | Some (_, question_tail) ->
...
val generated_compressed_soa_packet_subset_applicable :
  input:list FStar.UInt8.t ->
  Tot bool
...
let generated_compressed_srv_packet_fields input =
  match generated_uncompressed_question_answer_packet_fields input with
  | Some (qname_length, rr_name_length, rdata_length, rr_type, _, _, _) ->
      if rr_type = 33 &&
         rdata_length = 8 &&
         L.length input = 34 + qname_length + rr_name_length then
        match parse_header_bytes input with
        | Some (_, after_header) ->
            begin match DNS.Name.parse_qname 128 after_header with
            | Some (_, question_tail) ->
                if L.length question_tail < 4 then
                  ...
                  | Some (_, rr_tail) ->
                      ...
                      | _rt_hi :: _rt_lo ::
                        ...
                        _port_hi :: _port_lo ::
                        ptr_hi :: ptr_lo :: [] ->
                          let target_offset = 32 + qname_length + rr_name_length in
                          let target_name_pointer = [ptr_hi; ptr_lo] in
                          begin match DNS.Name.parse_qname_compressed
                                  128
                                  input
                                  target_offset
                                  target_name_pointer with
                          | Some (_, []) ->
                              let ptr_hi_nat = FStar.UInt8.v ptr_hi in
                              let ptr_lo_nat = FStar.UInt8.v ptr_lo in
                              if ptr_hi_nat >= 192 then begin
...
val generated_compressed_srv_packet_subset_applicable :
  input:list FStar.UInt8.t ->
  Tot bool

let generated_compressed_srv_packet_subset_applicable input =
  match generated_compressed_srv_packet_fields input with
  | Some (qname_length, rr_name_length, ptr_hi, ptr_lo) ->
      qname_length > 0 &&
      qname_length <= 255 &&
      rr_name_length > 0 &&
      ...
      ptr_hi <= 255 &&
      ptr_lo <= 255 &&
      L.length input = 34 + qname_length + rr_name_length
  | None -> false
...
val validate_generated_uncompressed_question_two_a_answer_packet_subset_buffer :
  buffer:LowStar.Buffer.buffer FStar.UInt8.t ->
  len:FStar.UInt32.t ->
  bytes:list FStar.UInt8.t{
    L.length bytes == FStar.UInt32.v len /\
    generated_uncompressed_question_two_a_answer_packet_subset_applicable bytes == true
  } ->
  Stack bool
    (requires (fun h0 ->
      LowStar.Buffer.live h0 buffer /\
...
let validate_generated_uncompressed_question_two_a_answer_packet_subset_buffer buffer len bytes =
  match generated_uncompressed_question_two_a_answer_packet_fields bytes with
  | Some (qname_length_nat, first_rr_name_length_nat, second_rr_name_length_nat) ->
      assert (qname_length_nat > 0);
      assert (qname_length_nat <= 255);
      assert (first_rr_name_length_nat > 0);
      assert (first_rr_name_length_nat <= 255);
      ...
      let qname_length = FStar.UInt32.uint_to_t qname_length_nat in
      assert (FStar.UInt32.v qname_length == qname_length_nat);
      let first_rr_name_length =
        FStar.UInt32.uint_to_t first_rr_name_length_nat in
      assert (FStar.UInt32.v first_rr_name_length == first_rr_name_length_nat);
      let second_rr_name_length =
        FStar.UInt32.uint_to_t second_rr_name_length_nat in
      assert (FStar.UInt32.v second_rr_name_length == second_rr_name_length_nat);
      EPR.check_dns_uncompressed_question_two_a_answer_packet
        qname_length
        first_rr_name_length
        second_rr_name_length
        buffer
        len
  | None ->
      false
...
let validate_generated_compressed_name_rdata_packet_subset_buffer buffer len bytes =
  ...
  | Some (qname_length_nat, rr_name_length_nat, rr_type_nat, ptr_hi_nat, ptr_lo_nat) ->
      assert (qname_length_nat > 0);
      assert (qname_length_nat <= 255);
      assert (rr_name_length_nat > 0);
      assert (rr_name_length_nat <= 255);
      ...
      let expected_rtype = FStar.UInt32.uint_to_t rr_type_nat in
      assert (FStar.UInt32.v expected_rtype == rr_type_nat);
      let rdata_name_ptr_hi_value = FStar.UInt32.uint_to_t ptr_hi_nat in
      assert (FStar.UInt32.v rdata_name_ptr_hi_value == ptr_hi_nat);
      let rdata_name_ptr_lo_value = FStar.UInt32.uint_to_t ptr_lo_nat in
      assert (FStar.UInt32.v rdata_name_ptr_lo_value == ptr_lo_nat);
...
val validate_generated_compressed_soa_packet_subset_buffer :
  buffer:LowStar.Buffer.buffer FStar.UInt8.t ->
  len:FStar.UInt32.t ->
  bytes:list FStar.UInt8.t{
    L.length bytes == FStar.UInt32.v len /\
    generated_compressed_soa_packet_subset_applicable bytes == true
  } ->
  Stack bool
    (requires (fun h0 ->
      LowStar.Buffer.live h0 buffer /\
...
let validate_generated_compressed_soa_packet_subset_buffer buffer len bytes =
  ...
  | Some (mname_compressed, qname_length_nat, rr_name_length_nat, other_name_length_nat, ptr_hi_nat, ptr_lo_nat) ->
      ...
      if mname_compressed then
        ...
        EPR.check_dns_uncompressed_question_compressed_soa_rname_answer_packet
          qname_length
          rr_name_length
          other_name_length
          ptr_hi_value
...
val validate_generated_compressed_srv_packet_subset_buffer :
  ...
  bytes:list FStar.UInt8.t{
    L.length bytes == FStar.UInt32.v len /\
    generated_compressed_srv_packet_subset_applicable bytes == true
  } ->
  Stack bool
    (requires (fun h0 ->
      LowStar.Buffer.live h0 buffer /\
...
let validate_generated_compressed_srv_packet_subset_buffer buffer len bytes =
  match generated_compressed_srv_packet_fields bytes with
  | Some (qname_length_nat, rr_name_length_nat, ptr_hi_nat, ptr_lo_nat) ->
      assert (qname_length_nat > 0);
      assert (qname_length_nat <= 255);
      assert (rr_name_length_nat > 0);
      ...
      let rr_name_length = FStar.UInt32.uint_to_t rr_name_length_nat in
      assert (FStar.UInt32.v rr_name_length == rr_name_length_nat);
      let target_name_ptr_hi_value = FStar.UInt32.uint_to_t ptr_hi_nat in
      assert (FStar.UInt32.v target_name_ptr_hi_value == ptr_hi_nat);
      let target_name_ptr_lo_value = FStar.UInt32.uint_to_t ptr_lo_nat in
      assert (FStar.UInt32.v target_name_ptr_lo_value == ptr_lo_nat);
      EPR.check_dns_uncompressed_question_compressed_srv_answer_packet
        qname_length
        rr_name_length
...
let validate_generated_subset_gate_buffer buffer len bytes =
  if generated_edns0_opt_additional_packet_subset_applicable bytes then
    ...
    validate_generated_compressed_srv_packet_subset_buffer buffer len bytes
  else if generated_uncompressed_question_two_a_answer_packet_subset_applicable bytes then
    validate_generated_uncompressed_question_two_a_answer_packet_subset_buffer buffer len bytes
